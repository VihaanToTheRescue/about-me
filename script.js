(function() {
    ...
    function startMainContent() {
        ...
        mainContentElements.forEach(el => { el.style.display = ''; void el.offsetWidth; });
        if (headerName) {
        ...
        document.querySelectorAll('.theme-toggle, .typing, .filter-buttons, .socials-icons, .footer, .header, .section')
            .forEach(el => { el.style.opacity = '0'; void el.offsetWidth; });
    }
    ...
    function revealPasswordCharByChar(displayElement, actualInputElement, targetPassword, finalCallback) {
        ...
        // displayElement.textContent = "Enter Password: "; // Reset by createHackingShow
            ...
            if (charRevealIndex < targetPassword.length) {
                ...
                function animateCharReveal() {
                    if (attempts < maxAttemptsPerChar) {
                        ...
                        actualInputElement.value = currentRevealedPassword; // Update the input field's value
                        charRevealIndex++; setTimeout(revealNextChar, interCharRevealDelay);
                    }
                }
                animateCharReveal();
            } else {
                displayElement.innerHTML = `Enter Password: ${currentRevealedPassword}<span class="cursor">_</span>`; // Final state
                actualInputElement.value = targetPassword; // Ensure final value is correct
    ...
    async function showIpWarningAndProceed() {
        ...
        if (passwordPrompt) { // Ensure element exists
            // passwordPrompt.classList.remove('visible'); // classList handles opacity/transition if used
            passwordPrompt.style.display = 'none'; // Directly hide it
        }
    ...
    function createHackingShow() {
        ...
        // --- Make password prompt visible for the reveal ---
        passwordPrompt.style.display = 'flex'; // Or 'block'
        passwordPrompt.classList.add('visible'); // Ensure opacity/visibility is correct
        passwordText.innerHTML = 'Enter Password: <span class="cursor">_</span>'; // Reset prompt text
        passwordInput.value = ""; // Clear any previous incorrect attempts
        passwordInput.disabled = true; // Keep it disabled until the reveal function enables it
        // ---
        ...
        let lineIndex = 0; hackingCode.textContent = '';
        ...
        function displayCode() {
            if (lineIndex < codeLines.length) {
                ...
                if (isKeyFoundLine) {
                    passwordInput.disabled = false; // Enable for reveal function
                    revealPasswordCharByChar(passwordText, passwordInput, correctPassword, () => {
                        passwordInput.disabled = true; // Disable again after reveal
                ...
                setTimeout(() => {
                    passwordStatus.style.color = ''; // Reset color
                    hackingCode.classList.remove("visible");
                    passwordPrompt.classList.remove("visible");
                    passwordPrompt.style.display = 'none'; // Hide it before starting main content
    ...
    function handlePasswordInput() {
        ...
        passwordInput.addEventListener("input", () => {
            ...
            passwordText.innerHTML = `Enter Password: ${passwordInput.value}<span class="cursor">_</span>`;
        });
        passwordInput.addEventListener("keypress", (e) => {
            ...
            if (e.key === "Enter") {
                ...
                if (enteredValue === correctPassword) {
                    ...
                    passwordStatus.textContent = "Access Denied"; passwordInput.value = "";
                    // passwordText.innerHTML = 'Enter Password: <span class="cursor">_</span>'; // Already handled by createHackingShow reset
    ...
    if (splash) {
        ...
        setTimeout(() => {
            ...
            } else { isAuthenticated = true; startMainContent(); }
        }, splashDuration);
    } else {
        if (passwordPrompt) {
            passwordPrompt.classList.add("visible");
    ...
    function switchTheme(newTheme) {
        ...
        else createMatrixPasswordTransition("wave-transition", () => updateThemeUI("dark"));
    }
